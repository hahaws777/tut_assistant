...
def update_roadmap_leaves(
    ...
    prompt = (
        ...
        "1. active_node: which node index (0-based) is currently being discussed. "
        "Set to -1 if the user is only greeting, chatting, or asking general questions "
        "and has NOT started working on a specific problem yet.\n"
        "2. done_nodes: list of node indices where the full solution has been provided or the problem is completed.\n"
        "3. leaves: sub-step labels (English, short) for the ACTIVE node ONLY. "
        "Only include leaves that have actually been discussed or revealed in the conversation. "
        "Do NOT pre-generate future steps that haven't been talked about yet. "
        "If active_node is -1, leaves MUST be an empty array [].\n"
        "4. active_leaf: 0-based index of the leaf currently being worked on. "
        "-1 if no specific leaf is active or if leaves is empty.\n\n"
        "IMPORTANT: If the conversation is just greetings or topic overview with no specific problem work, "
